...
namespace heron
{
  ...
  class USVMPCPlant : public BasePlant<CONTROLLER_T>
  {
  public:
    using control_array = typename USVDynamics::control_array;
    using state_array = typename USVDynamics::state_array;
    using output_array = typename USVDynamics::output_array;

    USVMPCPlant(std::shared_ptr<CONTROLLER_T> controller, int hz, int optimization_stride)
        : BasePlant<CONTROLLER_T>(controller, hz, optimization_stride)
    {
      system_dynamics_ = std::make_shared<USVDynamics>();
    }
  ...
  protected:
    std::shared_ptr<USVDynamics> system_dynamics_;
